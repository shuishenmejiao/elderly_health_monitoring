...
class ChunkDoubleSourceSlider3(object):
    def __init__(self, filename, batchsize, chunksize, shuffle, crop=None, header=0, ram_threshold=5 * 10 ** 5):
    ...
    def feed_chunk(self):
        ...
        if self.total_size == 0:
            ChunkDoubleSourceSlider3.check_length(self)
        ...
        if self.total_size > self.ram:  # IF dataset is too large for memory
            ...
            for i in skip_idx:
                ...
                np_array = np.array(data)
                inputs, targets = np_array[:, :-1], np_array[:, -1]
                ...
                # providing sliding windows:
                for start_idx in range(0, max_batchsize, self.batchsize):
                    inp = inputs[start_idx:start_idx + self.batchsize]
                    tar = targets[start_idx:start_idx + self.batchsize].reshape(-1,1)
...
class DoubleSourceProvider4(object):
    ...
    def feed(self, inputs):
        ...
        # providing sliding windows:
        for start_idx in range(0, max_nofw, self.nofWindows):
            inp = inputs[start_idx:start_idx + self.nofWindows]
