...
def get_FP(target, prediction, threshold):
    ...
    assert (target.shape == prediction.shape)
    one = np.ones(target.shape[0])
    # target = np.clip(target, threshold, 0) / threshold
    # prediction = 1 - np.clip(prediction, threshold, 0) / threshold

    fp_array = np.logical_and(one-target, prediction) * 1.0
    fp = np.sum(fp_array)
...
def get_FN(target, prediction, threshold):
    ...
    fn_array = np.logical_and(target, one-prediction) * 1.0
    fn = np.sum(fn_array)
...
def get_TN(target, prediction, threshold):
    ...
    tn_array = np.logical_and(one-target, one-prediction) * 1.0
    tn = np.sum(tn_array)
