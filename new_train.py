# -*- coding: utf-8 -*-
"""
Created on Sat Feb 13 10:07:34 2021
...
from DataProvider import ChunkDoubleSourceSlider3
...
training_path = args.datadir + appliance_name + '/train.csv'
log('Training dataset: ' + training_path)

# path for validation data
validation_path = args.datadir + appliance_name + '/validation.csv'
log('Validation dataset: ' + validation_path)

# Defining object for training set loading and windowing provider (DataProvider.py)
tra_provider = ChunkDoubleSourceSlider3(filename=training_path,
...
val_provider = ChunkDoubleSourceSlider3(filename=validation_path,
                                        batchsize=args.batchsize,
                                        chunksize=CHUNK_SIZE,
                                        crop=args.crop_dataset,
                                        shuffle=False,
...
cost = -tf.reduce_mean(y_ * tf.log(tf.clip_by_value(y,1e-10,1.0))+tf.constant(args.prior)*(tf.constant(1.)-y_) * tf.log(tf.clip_by_value(tf.constant(1.)-y,1e-10,1.0)))
